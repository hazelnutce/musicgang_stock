...
export class StockPage extends Component {
  ...
  renderStock = (stocks) => {
    ...
    return(
      _.map(stocks,(stock) => {
        return(
          <StockDetail  key={stock.stockName} stockName={stock.stockName} itemCount={stock.itemCount} itemWarning={stock.itemWarning} itemDanger={stock.itemDanger} items={stock.items} _id={stock._id}/>
        )
      })
